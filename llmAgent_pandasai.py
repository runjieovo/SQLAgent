...
def execute_query(state: State):
    ...
    connection = pymysql.connect(
        ...
        user="root",
        password="88888888",
        database=state['database']  # 替换为你的数据库名
    )
    df = pd.read_sql(state['query'], connection)
    global query_result_df
    query_result_df = df
    print("=========================Query Output============================")
    print(df)
    print("=================================================================")
    connection.close()
    return {"result": str(df), "result_df": df}
...
def plot_result(state: State):
    ...
    resp += ['imgpth = plot(data)']
    resp = '\n'.join(resp).strip()
    print(resp)
    data = ast.literal_eval(state["result"])
    exec(resp)
    return {"imgpth": "plot.png"}
...
agent = Agent(query_result_df, memory_size=10)
plot_query = input(">The search results are given above, what do you want to plot: ")
agent.chat(plot_query)